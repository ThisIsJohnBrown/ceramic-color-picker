...
async function uploadImageToHosting(imagePath) {
  try {
    ...
    // Create a data URL (this works for small images)
...
// Function to create glaze records with working image uploads (LIMITED TO 3)
async function createGlazeRecordsWithImages() {
  console.log('Creating 3 glaze records with working image uploads...');
  ...
  for (const glaze of testGlazes) {
    try {
      ...
      const imageData = await uploadImageToHosting(glaze.image);
      ...
      if (imageData) {
        recordData['Sample Photos'] = [imageData];
        console.log(`✓ Image prepared for: ${glaze.name}`);
      } else {
        console.log(`⚠️  Image preparation failed for: ${glaze.name}`);
      }
...
// Function to create underglaze records with working image uploads (LIMITED TO 3)
async function createUnderglazeRecordsWithImages() {
  console.log('Creating 3 underglaze records with working image uploads...');
  ...
  for (const underglaze of testUnderglazes) {
    try {
      ...
      // Prepare the image for upload
      const imageData = await uploadImageToHosting(underglaze.image);
      ...
      // Add the image attachment if successful
      if (imageData) {
        recordData['Photo'] = [imageData];
        console.log(`✓ Image prepared for: ${underglaze.name}`);
      } else {
        console.log(`⚠️  Image preparation failed for: ${underglaze.name}`);
      }
...
// Main function
async function main() {
  console.log('🚀 Testing working image upload to Airtable...\n');
  ...
  try {
    ...
    // Create 3 glaze records with working image uploads
    const glazeResults = await createGlazeRecordsWithImages();
    
    // Create 3 underglaze records with working image uploads
    ...
    console.log('\n🎉 Working image upload test complete!');
    ...
    if (totalSuccess === 6) {
      ...
      console.log('\n💡 Note: Images are uploaded as data URLs. For production use,');
      console.log('   consider uploading to a cloud service like Cloudinary or AWS S3 first.');
    } else {
      console.log('\n⚠️  Some records failed. Check the error messages above.');
    }
    
  } catch (error) {
    console.error('Error during working image upload test:', error.message);
  }
}
