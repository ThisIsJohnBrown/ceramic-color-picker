...
function createAttachment(imagePath) {
  try {
    ...
    // Create a proper attachment object for Airtable
    // Airtable expects an array of objects with url and filename
    return {
      url: `file://${fullPath}`, // Use file:// protocol for local files
      filename: fileName
    };
...
// Function to create glaze records with proper image uploads (LIMITED TO 3)
async function createGlazeRecordsWithImages() {
  console.log('Creating 3 glaze records with proper image uploads...');
...
// Function to create underglaze records with proper image uploads (LIMITED TO 3)
async function createUnderglazeRecordsWithImages() {
  console.log('Creating 3 underglaze records with proper image uploads...');
...
// Main function
async function main() {
  console.log('🔧 Testing fixed image upload to Airtable...\n');
  ...
  try {
    ...
    // Create 3 glaze records with proper image uploads
    const glazeResults = await createGlazeRecordsWithImages();
    
    // Create 3 underglaze records with proper image uploads
    ...
    console.log('\n🎉 Fixed image upload test complete!');
    ...
    console.error('Error during fixed image upload test:', error.message);
  }
}
